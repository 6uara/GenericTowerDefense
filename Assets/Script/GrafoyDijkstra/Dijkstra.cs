...
public class Dijkstra
{
    ...
    public void Dijks(Grafo grafo, int source)
    {
        ...
        distance[source] = 0;
        nodos1[source] = nodos2[source] = grafo.Etiqs[source].GetComponent<PointInfo>().ID;
        ...
        for (int count = 0; count < verticesCount - 1; ++count)
        {
            ...
            for (int v = 0; v < verticesCount; ++v)
            {
                ...
                if (!shortestPathTreeSet[v] && Convert.ToBoolean(graph[u, v]) && distance[u] != int.MaxValue && distance[u] + graph[u, v] < distance[v])
                {
                    ...
                    nodos1[v] = grafo.Etiqs[u].GetComponent<PointInfo>().ID;
                    nodos2[v] = grafo.Etiqs[v].GetComponent<PointInfo>().ID;
                }
            }
        }
        ...
        int nodOrig = grafo.Etiqs[source].GetComponent<PointInfo>().ID;
        for (int i = 0; i < verticesCount; i++)
        {
            if (nodos1[i] != -1)
            {
