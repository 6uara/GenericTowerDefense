...
public class Dijkstra : MonoBehaviour
{
        ...
        public static void Dijks(Grafo grafo, int source)
        {
            ...
            for (int count = 0; count < verticesCount - 1; ++count)
            {
                ...
                for (int v = 0; v < verticesCount; ++v)
                {
                    ...
                    if (!shortestPathTreeSet[v] && System.Convert.ToBoolean(graph[u, v]) && distance[u] != int.MaxValue && distance[u] + graph[u, v] < distance[v])
                    {
